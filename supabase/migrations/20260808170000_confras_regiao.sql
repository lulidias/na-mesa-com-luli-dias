-- A Confra das Confras — produtor e região em campos separados

alter table public.confras_garrafas add column if not exists regiao text;

drop view if exists public.confras_vinhos_publico;
create view public.confras_vinhos_publico as
  select g.id, g.vinho, g.produtor, g.regiao, g.safra, g.formato, g.tipo, g.pais, g.litros, g.vagas, g.foto_url,
...
drop function if exists public.confras_add_garrafa(uuid, text, text, text, text, numeric, int, text, text);
...
begin
  ...
  insert into confras_garrafas (vinho, produtor, regiao, safra, formato, litros, vagas, tipo, pais, criado_por)
  values (trim(p_vinho), nullif(trim(p_produtor), ''), nullif(trim(coalesce(p_regiao, '')), ''),
          nullif(trim(p_safra), ''), p_formato, p_litros, greatest(p_vagas, 1),
          coalesce(nullif(trim(p_tipo), ''), 'Tinto'), nullif(trim(coalesce(p_pais, '')), ''), p_participante)
  returning id into v_id;
  insert into confras_garrafa_membros (garrafa_id, participante_id) values (v_id, p_participante);
  return v_id;
end $$;
grant execute on function public.confras_add_garrafa(uuid, text, text, text, text, numeric, int, text, text, text) to anon, authenticated;
